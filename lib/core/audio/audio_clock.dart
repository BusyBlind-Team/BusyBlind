...
/// v0.1 用单调时钟实现，并预留输出延迟 L_out 与用户校准偏移 L_user 的补偿位；
...
abstract class AudioClock {
  ...
  /// 用户校准偏移（教程中跟拍 16 次取中位数得到）。
  int get userOffsetUs;
  set userOffsetUs(int us);
...
class SystemAudioClock implements AudioClock {
  ...
  int touchToAudioUs(int rawTouchUs) {
    ...
    return rawTouchUs + _touchMapUs - (_outputLatency + _userOffset);
  }
  ...
  @override
  int get userOffsetUs => _userOffset;
  @override
  set userOffsetUs(int us) => _userOffset = us;
