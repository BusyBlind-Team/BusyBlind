...
```bash
flutter test                        # 35 个测试（时钟调度 / 六个修行 / 视觉快照 / 本地存储 / widget 冒烟）
flutter analyze                     # 静态检查（当前无错误警告）
dart run tool/gen_sounds.dart       # 需 FFmpeg；合成临时 PCM 后输出 16 个 MP3 并自动清理中间文件
```
...
音效全部为脚本合成的 MP3 占位（参数对齐声音语义表），正式音效直接替换 `assets/sfx/` 同名文件即可。
...
**第 10 轮（2026-09-07）· 终检**

- 音效合成脚本确定性验证：重新生成零 diff。
- 全量验证：analyze 零 issue、29 测全绿、CI 绿、APK 产物正常产出。打 tag `v0.1.0`。
- 10 轮累计：修复真实缺陷 6 处（调度器事件重放 / 返回丢结果 / 结算无返回 / 打坐后台计时 / 静坐未满发修为 / 钓花状态机吊死 + 写盘竞态），机制建设 3 项（CI 守门、APK 产物、回归测试 ×3），包体 -47%。待办与建议全部归档在上方"已知待办"。

**第 11 轮（2026-09-07）· 美术与已知待办 2**

- 完成：16 个 WAV 全部切换为 MP3，声音 key 与玩法调用不变；2,376,370 B → 446,774 B（-81.2%），生成脚本经从头复现验证零 diff。
- 美术：静坐、木鱼、数雨、听潮、钓花、过河拥有各自的竖屏氛围底图、原生动态前景与状态反馈；横屏、窄屏和减少动态效果模式均有覆盖。
- 架构：`PracticeSession` 新增统一视觉刷新通知和释放接口，宿主负责监听并在退出时按顺序释放。
- 详见 [`docs/已知待办2与修行界面完成报告.md`](docs/已知待办2与修行界面完成报告.md)。

**已知待办（按优先级）**

1. AudioClock 换原生音频渲染时钟（接口已留好，短音效调度从毫秒级提升到采样级）——**押后**：等真机试听验证当前方案是否真的是瓶颈。
2. ✅ 已完成：16 个音效从 WAV 切换为 MP3，资产 2,376,370 B → 446,774 B（-81.2%）。
3. ✅ 已完成：打坐反挂机第三重——`MotionGate` 加速度滑动窗口占空比判定（sensors_plus），走动暂停计时、安静约 5 秒自动恢复，无传感器平台静默降级。
4. 好友功能接入 Supabase（全 app 唯一联网功能）——**等后端项目**：需要 Supabase 项目 URL/anon key（或授权新建）。
5. 手动结束交互（听潮/钓花右上角"结束"入口）待设计确认。
6. 首启教程（声音语义表→60 秒试玩→校准）——✅ 已完成：三段式 `TutorialPage`，`tutorialDone` 持久化门控，语义表每条可现场试听。
...
lib/
  ...
  widgets/           Low-Poly 按钮 / 立绘占位 / 修行场景 / 曲线 / 花瓣
assets/images/       六张修行 WebP 氛围底图
tool/gen_sounds.dart FFmpeg MP3 音效合成脚本
test/                时钟调度 / 六个修行 / 视觉快照 / 商店 / widget 冒烟
```
